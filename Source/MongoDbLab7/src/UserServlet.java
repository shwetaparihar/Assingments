...
public class UserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		MongoClientURI uri = new MongoClientURI("mongodb://dbuser:password@ds019708.mlab.com:19708/aselab7");
		MongoClient client = new MongoClient(uri);

		DB db = client.getDB(uri.getDatabase());
		DBCollection users = db.getCollection("user");
		BasicDBObject query = new BasicDBObject();
		query.put("user",request.getParameter("user"));// request.getParameter("name"));
...
//	}
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		BasicDBObject query = new BasicDBObject();
		query.put("user", params.get("user"));
		WriteResult result = userCollection.remove(query);	
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		newDocument.put("password", params.get("password"));
		newDocument.put("user", params.get("user"));
		
		BasicDBObject searchQuery = new BasicDBObject().append("user", params.get("user"));
		
		WriteResult result = userCollection.update(searchQuery, newDocument);
